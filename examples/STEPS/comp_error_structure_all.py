...
if __name__ == '__main__':
    ...
    predictions = predictions.where(da.isfinite(predictions), other = -15)
    
    dt = events_loop(predictions, observations)
    dt.to_netcdf(f'data/results_stratiform_events1.nc')

    ######### Convective events ############
    predictions = xr.open_dataarray('data/predictions_convective.zarr',
                                    chunks = {'central_time': 1, 'lead_time': 1, 'member': -1, 'y': -1, 'x': -1}).where(evaluation_mask, drop = True)
    ...
    dt = events_loop(predictions, observations)
    dt.to_netcdf(f'data/results_convective_events1.nc')
